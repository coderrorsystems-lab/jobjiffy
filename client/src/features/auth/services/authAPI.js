...
export async function registerProfessional(profile) {
  // Server expects professional registration under /api/auth/professional/register
...
export async function login(email, password, role ) {
  ...
  try {
    ...
    // Store tokens and user role if provided
    ...
    // Store user role from response (important for admin panel access!)
    // if (response.data.role) {
    //   console.log('Storing user role:', response.data.role);
...
export async function registerUser(userData) {
  // Normalize phone (remove spaces) and use server-expected field names
  ...
  try {
    const response = await apiClient.post('/api/auth/user/register', {
      ...
      otp: userData.otp // Include OTP for email verification
    });
